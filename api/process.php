...
function bookCalendar() {
	...
	$bsql	= "SELECT * FROM tbl_reservations WHERE rdate ='$bkdate' AND ucount='$ucount'";
	$bresult = dbQuery($bsql);
	if (dbNumRows($bresult) > 0) {
		$errorMessage = 'Sorry, this slot is booked already. Please try another slot/room.';
	...
	$sql = "INSERT INTO tbl_reservations (uid, ucount, rdate, status, comments, bdate) 
			VALUES ($userId, $ucount, '$bkdate', 'PENDING', '$comment', NOW())";
	dbQuery($sql);
	
	//send email on registration confirmation
	$bodymsg = "User $name booked the date slot on $bkdate. Requesting you to please take further action on user booking.";
	$data = array('to' => 'khasan550@gmail.com', 'sub' => 'Booking on $rdate.', 'msg' => $bodymsg);
	//send_email($data);
